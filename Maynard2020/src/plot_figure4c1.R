...
# df <- read.table("output/results/TH266/hFDR-wilcox-celltype1-Tumor-immune-PathSeq-spike-Bacteria-any-.5-plate-up-0.5.tsv", header=TRUE)
...
df$diffexpressed <- "NO"
df$diffexpressed[df$summary.AUC > 0.5 & df$unadjp < 0.05] <- "UP"
df$diffexpressed[df$summary.AUC < 0.5 & df$unadjp < 0.05] <- "DOWN"

df$delabel <- NA
df$delabel[df$diffexpressed != "NO"] <- df$name[df$diffexpressed != "NO"]

mycolors <- c("red", "red", "grey")
names(mycolors) <- c("DOWN", "UP", "NO")

ggplot(df, aes(x=summary.AUC, y=-log10(unadjp), color=diffexpressed, label=delabel)) +
  ...
  ggtitle("TH231 stroma versus non-stroma cells") +
  ylab("-log10(P)") +
  xlab("AUC")
...
ggsave(snakemake@output[[1]], width=2.75, height=3, units = "in")
