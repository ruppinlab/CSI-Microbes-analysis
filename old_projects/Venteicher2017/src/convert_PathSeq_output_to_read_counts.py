...
for _, sample in samples.iterrows():
    try:
        filename = snakemake.params[0].format(sample.patient, sample.name)
        pathseq_df = pd.read_csv(filename, sep="\t")
        pathseq_df["sample"] = sample.name
        #print(pathseq_df.loc[pathseq_df.name == "Plasmodium", "unambiguous"])
        # print(pathseq_df.loc[pathseq_df.name == "Plasmodium"])
        if pathseq_df.name.str.contains("Plasmodium_berghei_ANKA").any():
            plasmodium_reads = pathseq_df.loc[pathseq_df.name == "Plasmodium", "unambiguous"].values[0]
            plasmodium_berghei_reads = pathseq_df.loc[pathseq_df.name == "Plasmodium_berghei", "unambiguous"].values[0]
            plasmodium_berghei_ANKA_reads = pathseq_df.loc[pathseq_df.name == "Plasmodium_berghei_ANKA", "unambiguous"].values[0]
            pathseq_df.loc[pathseq_df.name == "Plasmodium", "unambiguous"] = plasmodium_reads - plasmodium_berghei_ANKA_reads
            pathseq_df.loc[pathseq_df.name == "Plasmodium_berghei", "unambiguous"] = plasmodium_berghei_reads - plasmodium_berghei_ANKA_reads
        #print(pathseq_df.loc[pathseq_df.name == "Plasmodium", "unambiguous"])
