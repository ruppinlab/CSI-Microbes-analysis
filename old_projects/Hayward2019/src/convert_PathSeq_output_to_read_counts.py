...
for _, sample in samples.iterrows():
    try:
        ...
        print(pathseq_df.loc[pathseq_df["name"] == "Chlamydia"])
        pathseq_df["sample"] = sample.name
        pathseq_df = pathseq_df.loc[pathseq_df["type"] == tax_level]
        pathseq_df = pathseq_df.drop(columns=["tax_id", "taxonomy", "type", "kingdom", "score", "score_normalized", "reference_length"])
        if pathseq_df.empty:
            pathseq_df = pd.DataFrame(data={"sample": [sample.name], "name": ["placeholder"], "reads": [0]})
