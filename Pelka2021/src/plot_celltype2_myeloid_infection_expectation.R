...
# df <- read.table("output/expected_actual_infected_cells_per_celltype2.tsv", sep="\t", header=TRUE)
df <- df[df$celltype1 == "Myeloid",]
n.rows <- dim(df)[1]
condition <- c(rep("expected", n.rows), rep("infected", n.rows))
celltype <- rep(df$celltype2, 2)
...
ggplot(df, aes(fill=condition, y=n.cells, x=celltype)) +
    ...
    ggtitle("Myeloid cell sub-types (Pelka2021)")
