...
condition <- c(rep("Bacteroides", 5))
celltype <- c(rep(c("Myeloid", "TNKILC", "B", "Tumor", "Stromal"), 1))
positive.percent <- c(Myeloid.Bacteroides, Tcell.Bacteroides, Bcell.Bacteroides, Epithelial.Bacteroides, Stromal.Bacteroides)

df <- data.frame(condition, celltype, positive.percent)

df$condition <- factor(df$condition, levels=c("Bacteroides"))
