...
calculate.Fishers.exact <- function(sce, chemistry.of.interest) {
  counts <- counts(sce[,(sce[["chemistry"]] == chemistry.of.interest)])
  celltype.of.interest.positive <- sum(colSums(counts) >= 1)
  ci.total <- dim(sce[,(sce[["chemistry"]] == chemistry.of.interest)])[[2]]
  counts <- counts(sce[,(sce[["chemistry"]] != chemistry.of.interest)])
  celltype.comparison.positive <- sum(colSums(counts) >= 1)
  cc.total <- dim(sce[,(sce[["chemistry"]] != chemistry.of.interest)])[[2]]
  celltype.of.interest.negative <- ci.total-celltype.of.interest.positive
  celltype.comparison.negative <- cc.total-celltype.comparison.positive
  ...
  return(c("p.value"=f.test$p.value[[1]], "summary.odds.ratio"=f.test$estimate[[1]], "or.ci.low"=f.test$conf.int[[1]], "or.ci.high"=f.test$conf.int[[2]], "n.reads"=sum(counts(sce)), "n.positive.cells.of.interest"=celltype.of.interest.positive, "n.negative.cells.of.interest"=celltype.of.interest.negative, "n.positive.cells.comparison"=celltype.comparison.positive,
...
out <- calculate.Fishers.exact(sce, "v3")
print(out)
