...
out_df = df.groupby(["patient", "Is_Tumor", "10x_chemistry"]).apply(lambda x: (x.loc[(x[read_df.columns] >= min_umis).any(axis=1)].shape[0]/x.shape[0])*100)
out_df.to_csv(snakemake.output[1], sep="\t")
name = "% Cells with >= {} UMIs".format(min_umis)
out_df.name = name
out_df = out_df.reset_index()
print(out_df.sort_values(by=name))
sns.boxplot(x="10x_chemistry", y=name, hue="Is_Tumor", data=out_df)
