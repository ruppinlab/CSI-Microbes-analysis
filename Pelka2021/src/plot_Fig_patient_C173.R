...
condition <- c(rep("Fusobacterium", 5), rep("Bacteroides", 5))
celltype <- c(rep(c("Myeloid", "TNKILC", "B", "Epithelial", "Stromal"), 2))
positive.percent <- c(Myeloid.Fusobacterium, Tcell.Fusobacterium, Bcell.Fusobacterium, Epithelial.Fusobacterium, Stromal.Fusobacterium, Myeloid.Bacteroides, Tcell.Bacteroides, Bcell.Bacteroides, Epithelial.Bacteroides, Stromal.Bacteroides)

df <- data.frame(condition, celltype, positive.percent)

df$condition <- factor(df$condition, levels=c("Fusobacterium", "Bacteroides"))
