...
get.nreads <- function(sce, chemistry.of.interest, microbe.of.interest) {
  nreads <- sum(counts(sce[,(sce[["chemistry"]] == chemistry.of.interest)])[microbe.of.interest,])
  return(nreads)
}

get.nreads.any <- function(sce, chemistry.of.interest) {
  counts <- counts(sce[,(sce[["chemistry"]] == chemistry.of.interest)])
  nreads <- sum(colSums(counts))
  return(nreads)
}


v2.infected.all <- get.nreads.any(sce, "v2")
v3.infected.all <- get.nreads.any(sce, "v3")
v2.infected.Fusobacterium <- get.nreads(sce, "v2", "Fusobacterium")
v3.infected.Fusobacterium <- get.nreads(sce, "v3", "Fusobacterium")
v2.infected.Bacteroides <- get.nreads(sce, "v2", "Bacteroides")
v3.infected.Bacteroides <- get.nreads(sce, "v3", "Bacteroides")


condition <- c(rep("All Genera", 2), rep("Fusobacterium", 2), rep("Bacteroides", 2))
chemistry <- c(rep(c("3' v2", "3' v3"), 3))
nreads <- c(v2.infected.all, v3.infected.all, v2.infected.Fusobacterium, v3.infected.Fusobacterium, v2.infected.Bacteroides, v3.infected.Bacteroides)
...
ggplot(df, aes(fill=chemistry, y=nreads, x=genera)) +
    ...
    ylab("Total number of UMIs mapped to") +
    xlab(NULL) +
