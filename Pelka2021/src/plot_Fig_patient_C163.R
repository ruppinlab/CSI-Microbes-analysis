...
Myeloid.Bacteroides <- get.positive.percent(sce, "Myeloid", "Bacteroides")
Tcell.Bacteroides <- get.positive.percent(sce, "TNKILC", "Bacteroides")
Bcell.Bacteroides <- get.positive.percent(sce, "B", "Bacteroides")
Epithelial.Bacteroides <- get.positive.percent(sce, "Epithelial", "Bacteroides")
Stromal.Bacteroides <- get.positive.percent(sce, "Stromal", "Bacteroides")

Myeloid.Campylobacter <- get.positive.percent(sce, "Myeloid", "Campylobacter")
Tcell.Campylobacter <- get.positive.percent(sce, "TNKILC", "Campylobacter")
Bcell.Campylobacter <- get.positive.percent(sce, "B", "Campylobacter")
Epithelial.Campylobacter <- get.positive.percent(sce, "Epithelial", "Campylobacter")
Stromal.Campylobacter <- get.positive.percent(sce, "Stromal", "Campylobacter")


condition <- c(rep("Fusobacterium", 5), rep("Bacteroides", 5), rep("Campylobacter", 5))
celltype <- c(rep(c("Myeloid", "TNKILC", "B", "Tumor", "Stromal"), 3))
positive.percent <- c(Myeloid.Fusobacterium, Tcell.Fusobacterium, Bcell.Fusobacterium, Epithelial.Fusobacterium, Stromal.Fusobacterium, Myeloid.Bacteroides, Tcell.Bacteroides, Bcell.Bacteroides, Epithelial.Bacteroides, Stromal.Bacteroides, Myeloid.Campylobacter, Tcell.Campylobacter, Bcell.Campylobacter, Epithelial.Campylobacter, Stromal.Campylobacter)

df <- data.frame(condition, celltype, positive.percent)

df$condition <- factor(df$condition, levels=c("Fusobacterium", "Bacteroides", "Campylobacter"))
