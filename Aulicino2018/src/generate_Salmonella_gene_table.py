...
for _, cell in cells.iterrows():
    ...
    if genome == "LT2":
        df["geneName"] = df["attributes"].apply(lambda x: x.split(";")[2].replace("Name=", ""))
    elif genome == "D23580":
        df["geneName"] = df["attributes"].apply(lambda x: x.split(";")[1].replace("Name=", ""))
    #df["geneID"] = df["attributes"].apply(lambda x: x.split(";")[0].replace("ID=", ""))
    df = df.set_index(keys="geneName")
