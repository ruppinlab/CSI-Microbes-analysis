...
get.positive.percent <- function(sce, microbe.of.interest, sample.of.interest) {
...
Salmonella.exposed <- get.positive.percent(sce, "Salmonella", "GSM3454529")
Salmonella.unexposed <- get.positive.percent(sce, "Salmonella", "GSM3454528")


df <- data.frame(condition=c("unexposed (3' v2)", "exposed (3' v2)"), percentage=c(Salmonella.unexposed, Salmonella.exposed))
...
ggplot(df, aes(fill=condition, y=percentage, x=condition)) +
