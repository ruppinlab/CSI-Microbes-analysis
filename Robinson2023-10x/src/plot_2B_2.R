...
ci.positive <- sum(counts(sce[,celltype == celltype.of.interest])[microbe.of.interest,] > 1)
ci.total <- dim(sce[,celltype == celltype.of.interest])[[2]]
comparison.positive <- sum(counts(sce[,celltype == celltype.comparison])[microbe.of.interest,] > 1)
...
df$celltype <- factor(df$celltype, levels=c("Jurkat", "HCT116"))
