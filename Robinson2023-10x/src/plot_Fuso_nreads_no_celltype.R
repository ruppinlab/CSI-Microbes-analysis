...
uninfected <- get.nreads(sce, "SCAF2961_1_Uninfected")
HK <- get.nreads(sce, "SCAF2962_2_HK")
live.3p <- get.nreads(sce, "SCAF2963_3_Live")
live.5p <- get.nreads(sce, "SCAF2965_5_Live")
condition <- c(rep("Uninfected", 1), rep("Heat-Killed Fn", 1), rep("Live Fn (3' v3)", 1), rep("Live Fn (5')", 1))
nreads <- c(uninfected, HK, live.3p, live.5p)

df <- data.frame(condition, nreads)
...
ggplot(df, aes(fill=condition, y=nreads, x=condition)) +
    ...
    ylab("number of reads from Fusobacterium") +
    xlab(NULL) +
