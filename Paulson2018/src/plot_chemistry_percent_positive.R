...
Tumor.3p.post <- get.positive.percent(sce, "Tumor", "Human_polyomavirus_5", "Day615Tumor")
Tumor.3p.pre <- get.positive.percent(sce, "Tumor", "Human_polyomavirus_5", "PreRxTumor")
Tumor.5p.post <- get.positive.percent(sce, "Tumor", "Human_polyomavirus_5", "RelapseD565Tumor")
condition <- c("2586 pre", "2586 post", "9245 post")
chemistry <- c("3' v2", "3' v2", "5'")
positive.percent <- c(Tumor.3p.pre, Tumor.3p.post, Tumor.5p.post)

df <- data.frame(condition, chemistry, positive.percent)
df$condition <- factor(df$condition, levels=c("2586 pre", "2586 post", "9245 post"))

ggplot(df, aes(fill=chemistry, y=positive.percent, x=condition)) +
    ...
    xlab(NULL) +
    ylab("% of tumor cells with >= 1 UMI to Merkel polyomavirus") +
