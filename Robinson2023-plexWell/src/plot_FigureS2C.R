...
sce <- sce[, sce$Condition %in% c("Uninfected", "ERCC only", "Heat Killed", "Infected")]
...
fig.1a <- ggboxplot(df, x = "Condition", y = "Fusobacterium", color = "Condition", add = "jitter") +
  ...
  scale_color_manual(values=c("#00A087", "#4DBBD5FF","#E64B35FF", "#3C5488")) + # inspired by scale_color_npg
