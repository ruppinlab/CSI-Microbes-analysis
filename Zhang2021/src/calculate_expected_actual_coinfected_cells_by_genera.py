...
def get_expected_coinfected_cells(df, genera1, genera2, cutoff):
    # calculate the expected number of infected cells per cell-type
    n_expected_df = df.groupby(["patient", "sample"]).apply(lambda x: ((x.loc[(x[genera1] >= cutoff)].shape[0]/x.shape[0])*(x.loc[(x[genera2] >= cutoff)].shape[0]/x.shape[0]))*x.shape[0])
...
def single_sample_g1_g2_hypergeom(df, sample, g1, g2, cutoff):
    ...
    N = sample_df.loc[sample_df[g2] >= cutoff].shape[0]
    k = sample_df.loc[(sample_df[g1] >= cutoff) & (sample_df[g2] >= cutoff)].shape[0]
    M = sample_df.shape[0]
    hpd = hypergeom(M, n, N)
    p = hpd.pmf(k)
    return p

def calculate_p_value(df, g1, g2, cutoff):
    samples = df["sample"].unique()
    # celltype_of_interest = "Myeloid"
    output = []
    for sample in samples:
        pval = single_sample_g1_g2_hypergeom(df, sample, g1, g2, cutoff)
        output.append(pd.Series({"sample": sample, "g1": g1, "g2": g2, "pvalue": pval}))
    return pd.concat(output, axis=1).T

def calculate_coocurrence_pvalues(df, g1, g2, cutoff):
    expected_coinfected_cells = get_expected_coinfected_cells(df, g1, g2, cutoff)
    coinfection_pvalues = calculate_p_value(df, g1, g2, cutoff)
...
for g1 in infecting_genera:
    for g2 in infecting_genera:
        if g1 <= g2:
            continue
        expected_coinfected_cells = get_expected_coinfected_cells(df, g1, g2, cutoff)
        coinfected_cells = get_n_coinfected_cells(df, g1, g2, cutoff)
        if expected_coinfected_cells["n_expected_infected"].sum() == 0:
            pval = None
        else:
            pval = calculate_coocurrence_pvalues(df, g1, g2, cutoff)
