...
for _, cell in cells.iterrows():
    try:
        #print(cell)
        filename = snakemake.params[0].format(cell["patient"], cell["sample"], cell["cell"])
        pathseq_df = pd.read_csv(filename, sep="\t")
        cell_name = "{}-{}".format(cell["sample"], cell["cell"])
